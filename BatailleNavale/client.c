...
void write_server_square(int sockfd, int buffer[2]) {
...
void recv_grid(int src[10][10], int sockfd) {
  int msg[10][10];
...
void take_turn(int sockfd) {
  ...
  while (1) {
    ...
    if (valid == 1) {
      int placement = getNumberInTab(buffer[1], lettres);
      int line = buffer[0] - '0';
      coup[0] = line;
      coup[1] = placement;
      write_server_square(sockfd, coup);
      memcpy(square, coup, 2 * sizeof(int));
      break;
    } else
...
void fill_data(int sockfd) {
  ...
  draw_grid_placement(grid_player);
  ...
  printf("Choisissez les quatre cases que vous voulez remplir (par exemple 6B-6C-6D-6E ou 0J-1J-2J-3J).\n");
...
int main(int argc, char * argv[]) {
  ...
  while (1) {
    ...
    if (!strcmp(msg, "FIG")) {
      ...
      fill_data(sockfd);
    } else if (!strcmp(msg, "WFG")) {
      printf("Votre adversaire remplit sa grille.\n");
    } else if (!strcmp(msg, "STP")) {
      printf("\nLE JEU COMMENCE ! \n");
    } else if (!strcmp(msg, "NTR")) {
      printf("\n Votre adversaire est en train de jouer. \n");
    } else if (!strcmp(msg, "HBT")) {
      printf("\033[41mVotre adversaire a touché votre flotte ! Reprenez vous moussaillon !\033[0m\n");
    } else if (!strcmp(msg, "PAT")) {
      printf("\n\033[43mVous avez coulé le porte avion de votre adversaire.\033[0m\n");
    } else if (!strcmp(msg, "PAC")) {
      printf("\n\033[43mVotre porte avion a été coulé ...\033[0m\n");
    } else if (!strcmp(msg, "SMT")) {
      printf("\n\033[46mVous avez coulé un des sous marins de votre adversaire !\033[0m\n");
    } else if (!strcmp(msg, "SMC")) {
      printf("\n\033[46mVotre sous marin a été coulé ...\033[0m\n");
    } else if (!strcmp(msg, "TOT")) {
      printf("\n\033[41mVous avez coulé le torpilleur de votre adversaire !\033[0m\n");
    } else if (!strcmp(msg, "TOC")) {
      printf("\n\033[41mVotre torpilleur a été coulé ...\033[0m\n");
    } else if (!strcmp(msg, "HNT")) {
      printf("\033[42mVotre adversaire a loupé sa cible ! Restons concentré !\033[0m \n");
    } else if (!strcmp(msg, "TRN")) {
      printf("\n\033[44mA votre tour !\033[0m \n");
      take_turn(sockfd);
    } else if (!strcmp(msg, "WIN")) {
      printf("\n\033[42mBRAVO SOLDAT ! VOUS AVEZ GAGNE !\033[0m \n");
      break;
    } else if (!strcmp(msg, "LOS")) {
